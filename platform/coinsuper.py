...
from aed_quant.const import COINSUPER
...
class CoinsuperRestAPI:
    ...
    async def revoke_orders(self, order_ids):
        ...
        """
        data = {
            "orderNoList": ",".join(order_ids)
        }
        success, error = await self.request("/api/v1/order/batchCancel", data)
        return success, error

    async def get_order_list(self, order_ids):
        """ 获取用户委托列表(单次查询最大限50条记录)
        @param order_ids 订单号id列表
        """
        data = {
            "orderNoList": ",".join(order_ids)
        }
    ...
    async def get_order_details(self, order_ids):
        """ 根据委托单号，查询委托单交易详情
        @param order_ids 订单号id列表
        """
        data = {
    ...
    async def get_open_order_ids(self, symbol=None, num=1000):
    ...
    async def get_history_orders(self, symbol, start, end, start_order_id=None, with_trade="true", size=100):
        """ 获取当前还未完全成交的订单信息
        @param symbol 交易对
        @param start 开始时间戳(毫秒)
        @param end 结束时间戳(毫秒)
        @param start_order_id 起始委托单号（查询结果中不含该单号的数据，不传则表示查第一页数据）
        ...
        if start_order_id:
            data["StartOrderNo"] = str(start_order_id)
...
class CoinsuperTrade:
    ...
    async def _initialize(self):
        ...
        """
        # 获取当前所有未完全成交的订单
        order_ids, error = await self._rest_api.get_open_order_ids(self._raw_symbol)
        if error:
    ...
    async def revoke_order(self, *order_ids):
        ...
        if len(order_ids) > 1:
            success, error = await self._rest_api.revoke_orders(order_ids)
            if error:
                return None, error
            if len(success["failResultList"]) > 0:
    ...
    async def get_open_order_ids(self):
        """ 获取未完全成交订单号列表
        """
        success, error = await self._rest_api.get_open_order_ids(self._raw_symbol)
        return success, error
    ...
    async def _update_order(self, order_info):
        ...
        order_id = str(order_info["orderNo"])
        ...
        if not order:
            info = {
                ...
                "symbol": self._symbol,
                "vwm": order_info["priceLimit"],
                "quantity": order_info["quantity"],
                "remain": order_info["quantityRemaining"],
                "bias_to_vwp": order_info["priceLimit"]
            }
            order = Order(**info)
            self._orders[order_id] = order
