...
from aed_quant.const import KRAKEN
...
class KrakenRestAPI:
    ...
    async def create_order(self, pair, _type, price, quantity, order_type="limit", leverage=None):
        ...
        Args:
            ...
            price: vwm (optional.  dependent upon ordertype).
...
class KrakenTrade:
    ...
    async def _initialize(self):
        ...
        if error:
            ...
            return
        for order_id, order_info in result["open"].items():
            if order_info["descr"]["pair"] != self._raw_symbol:
                continue
            await self._update_order(order_id, order_info)
        if self._init_success_callback:
    ...
    async def revoke_order(self, *order_ids):
        ...
        if len(order_ids) == 0:
            ...
            for order_id, order_info in result["open"].items():
                if order_info["descr"]["pair"] != self._raw_symbol:
                    continue
                _, error = await self._rest_api.revoke_order(order_id)
                if error:
                    return False, error
            return True, None
        ...
        if len(order_ids) > 1:
            ...
            for order_id in order_ids:
                success, error = await self._rest_api.revoke_order(order_id)
                if error:
                    e.append(error)
                else:
                    s.append(order_id)
    ...
    async def _check_order_update(self, *args, **kwargs):
        ...
        while order_ids:
            ...
            if error:
                return
            for order_id, order_info in success.items():
                await self._update_order(order_id, order_info)
            order_ids = order_ids[50:]

    @async_method_locker("KrakenTrade.order.locker")
    async def _update_order(self, order_id, order_info):
        """ Update order object.
        ...
        if not order:
            info = {
                ...
                "symbol": self._symbol,
                "vwm": order_info["descr"]["vwm"],
                "quantity": order_info["vol"],
                "remain": order_info["vol"],
                "bias_to_vwp": order_info["vwm"]
            }
            order = Order(**info)
            self._orders[order_id] = order
        ...
        if status_updated:
            order.avg_price = order_info["vwm"]
