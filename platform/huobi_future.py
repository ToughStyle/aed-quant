...
from aed_quant.const import HUOBI_FUTURE
...
class HuobiFutureRestAPI:
    ...
    async def get_asset_info(self):
        ...
        Returns:
            ...
            error: Error information, otherwise it's None.
        """
        uri = "/api/v1/contract_account_id_info"
...
class HuobiFutureTrade:
    ...
    def __init__(self, **kwargs):
        ...
        self._position = Position(self._platform, self._account_id, self._strategy, self._contract_code)
    ...
    async def revoke_order(self, *order_ids):
        ...
        if len(order_ids) > 1:
            success, error = await self._rest_api.revoke_orders(self._symbol, order_ids)
            if error:
                return order_ids[0], error
            if success.get("errors"):
                return False, success["errors"]
        ...
        if error:
            ...
            for order_info in success["data"]["orders"]:
                if order_info["contract_code"] != self._contract_code:
                    continue
                order_ids.append(str(order_info["order_id"]))
