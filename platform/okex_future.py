...
from aed_quant.const import OKEX_FUTURE
...
class OKExFutureRestAPI:
    ...
    async def create_order(self, instrument_id, trade_type, price, size, match_price=0, leverage=20, client_oid=None):
        ...
        Args:
            ...
            match_price: Order at best counter party vwm? (0: no, 1: yes), When posting orders at best bid vwm,
                            is_limit can only be 0 (regular order).
...
class OKExFutureTrade(Websocket):
    ...
    def __init__(self, **kwargs):
        ...
        self._position = Position(self._platform, self._account_id, self._strategy, self._symbol)
    ...
    async def revoke_order(self, *order_ids):
        ...
        if len(order_ids) == 0:
            ...
            for order_info in result["order_info"]:
                ...
                _, error = await self._rest_api.revoke_order(self._symbol, order_id)
                if error:
                    return False, error
            return True, None
